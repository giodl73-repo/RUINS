# RUINS - Partial Survival and Responsible Reconstruction

RUINS is a Knowledge Systems repo about partial survival: lost cities,
destroyed libraries, fragmentary texts, abandoned infrastructure, vanished
techniques, broken monuments, shipwrecks, erased settlements, and contested
reconstructions.

RUINS is paper-first. It does not start with code, schemas, crates, or generated
fixtures. Its first job is to define the research discipline: how to separate
surviving evidence, responsible inference, legend, modern projection, and
downstream cultural memory.

## Thesis

Most catalogs of ruins organize by place, period, or artifact type. RUINS
organizes by epistemic condition:

- what is gone,
- what survives,
- how surviving traces are interpreted,
- where claims become contested,
- what cannot be responsibly reconstructed,
- how the loss shaped later knowledge, identity, technology, or myth.

RUINS is not a travel guide, nostalgia archive, treasure-hunt index, or ancient
mysteries project. It is a knowledge system for handling absence carefully.

## Start Here

| If you're... | Go to |
|---|---|
| New to RUINS | [`MODULE.md`](MODULE.md) |
| Looking for the first paper | [`research/publications/panel-epistemology-of-ruins/`](research/publications/panel-epistemology-of-ruins/) |
| Reviewing the project | [`.roles/ROLE.md`](.roles/ROLE.md) |
| Scoring a paper | [`QUALITY_RUBRIC.md`](QUALITY_RUBRIC.md) |
| Working as an agent | [`agents/AGENT-ROSTER.md`](agents/AGENT-ROSTER.md) |
| Running repo-local skills | [`skills/README.md`](skills/README.md) |
| Picking first cases | [`research/seed-corpus.md`](research/seed-corpus.md) |

## Current Paper Chain

| Track | Paper | Status |
|---|---|---|
| Evidence and Absence | `panel-epistemology-of-ruins` | publication-ready |
| Evidence and Absence | `panel-library-alexandria-loss-memory` | publication-ready |
| Evidence and Absence | `panel-indus-nondecipherment` | publication-ready |
| Evidence and Absence | `panel-house-of-wisdom-institution-memory` | publication-ready |
| Evidence and Absence | `panel-dead-sea-scrolls-fragment-custody` | publication-ready |
| Evidence and Absence | `panel-herculaneum-papyri-virtual-unwrapping` | publication-ready |
| Evidence and Absence | `panel-maya-codices-destroyed-corpus` | publication-ready |
| Evidence and Absence | `panel-qin-book-burning-textual-destruction` | publication-ready |
| Evidence and Absence | `panel-vinland-saga-site-crosscheck` | publication-ready |
| Evidence and Absence | `panel-pompeii-cast-void-custody` | publication-ready |
| Evidence and Absence | `panel-laetoli-footprint-trackway-custody` | publication-ready |
| Evidence and Absence | `panel-archimedes-palimpsest-overwrite-custody` | publication-ready |
| Evidence and Absence | `panel-sinai-palimpsests-collection-overwrite` | publication-ready |
| Evidence and Absence | `panel-tollund-man-preserved-body-custody` | publication-ready |
| Evidence and Absence | `panel-lindow-man-preserved-body-custody` | publication-ready |
| Evidence and Absence | `panel-doggerland-submerged-landscape-proxy` | publication-ready |
| Evidence and Absence | `panel-atlit-yam-submerged-settlement-custody` | publication-ready |
| Evidence and Absence | `panel-dunhuang-library-cave-cache-custody` | publication-ready |
| Evidence and Absence | `panel-cairo-genizah-storeroom-cache-custody` | publication-ready |
| Evidence and Absence | `panel-akrotiri-catastrophic-snapshot-custody` | publication-ready |
| Evidence and Absence | `panel-joya-de-ceren-catastrophic-snapshot-custody` | publication-ready |
| Evidence and Absence | `panel-bamiyan-empty-niche-reconstruction-custody` | publication-ready |
| Evidence and Absence | `panel-mostar-bridge-reconstructed-void-custody` | publication-ready |
| Evidence and Absence | `panel-sutton-hoo-decayed-ship-imprint-custody` | publication-ready |
| Evidence and Absence | `panel-woodhenge-posthole-marker-custody` | publication-ready |
| Evidence and Absence | `panel-met-sphinx-lost-polychromy-trace-custody` | publication-ready |
| Evidence and Absence | `panel-parthenon-pigment-conservation-custody` | publication-ready |
| Evidence and Absence | `panel-warsaw-archival-reconstruction-custody` | publication-ready |
| Evidence and Absence | `panel-dresden-frauenkirche-archival-reconstruction-custody` | publication-ready |
| Evidence and Absence | `panel-hurrian-hymn-performance-reconstruction-custody` | publication-ready |
| Evidence and Absence | `panel-seikilos-epitaph-notated-performance-custody` | publication-ready |
| Evidence and Absence | `panel-mendesian-perfume-olfactory-reconstruction-custody` | publication-ready |
| Evidence and Absence | `panel-senetnay-balm-olfactory-reconstruction-custody` | publication-ready |
| Evidence and Absence | `panel-midas-beverage-gustatory-reconstruction-custody` | publication-ready |
| Evidence and Absence | `panel-babylonian-recipes-gustatory-reconstruction-custody` | publication-ready |
| Evidence and Absence | `panel-stonehenge-acoustic-scale-model-custody` | publication-ready |
| Evidence and Absence | `panel-erato-theatre-acoustic-reconstruction-custody` | publication-ready |
| Evidence and Absence | `panel-paleolithic-cave-lighting-custody` | publication-ready |
| Evidence and Absence | `panel-vr-firelight-paleolithic-art-custody` | publication-ready |
| Evidence and Absence | `panel-newgrange-solstice-alignment-custody` | publication-ready |
| Evidence and Absence | `panel-chaco-sun-dagger-digital-restoration-custody` | publication-ready |
| Evidence and Absence | `panel-seville-paradata-transparency-custody` | publication-ready |
| Evidence and Absence | `panel-oplontis-villa-paradata-transparency-custody` | publication-ready |
| Evidence and Absence | `panel-egtved-girl-isotopic-mobility-custody` | publication-ready |
| Evidence and Absence | `panel-amesbury-archer-isotopic-mobility-custody` | publication-ready |
| Evidence and Absence | `panel-cheddar-man-ancient-genomics-custody` | publication-ready |
| Evidence and Absence | `panel-kennewick-ancient-one-genomics-custody` | publication-ready |
| Evidence and Absence | `panel-neanderthal-dental-calculus-custody` | publication-ready |
| Evidence and Absence | `panel-milk-protein-dental-calculus-custody` | publication-ready |
| Evidence and Absence | `panel-ancestral-pueblo-fingerprint-custody` | publication-ready |
| Evidence and Absence | `panel-birch-pitch-masticated-resin-custody` | publication-ready |
| Evidence and Absence | `panel-ancient-latrine-parasite-custody` | publication-ready |
| Evidence and Absence | `panel-abu-hureyra-dung-proxy-custody` | publication-ready |
| Evidence and Absence | `panel-parchment-biocodicology-custody` | publication-ready |
| Evidence and Absence | `panel-stone-tool-usewear-residue-custody` | publication-ready |
| Evidence and Absence | `panel-dzudzuana-flax-fiber-custody` | publication-ready |
| Evidence and Absence | `panel-denisova-sedimentary-dna-custody` | publication-ready |
| Evidence and Absence | `panel-takarkori-ceramic-lipid-residue-custody` | publication-ready |
| Evidence and Absence | `panel-greenland-ice-core-lead-proxy-custody` | publication-ready |
| Evidence and Absence | `panel-santorini-cryptotephra-marker-custody` | publication-ready |
| Evidence and Absence | `panel-jerusalem-archaeomagnetic-destruction-custody` | publication-ready |
| Evidence and Absence | `panel-rapa-nui-lake-core-collapse-custody` | publication-ready |
| Evidence and Absence | `panel-madjedbebe-osl-sediment-age-custody` | publication-ready |
| Evidence and Absence | `panel-shell-midden-sclerochronology-seasonality-custody` | publication-ready |
| Evidence and Absence | `panel-mesa-verde-dendrochronology-timber-date-custody` | publication-ready |
| Evidence and Absence | `panel-obsidian-provenance-hydration-custody` | publication-ready |
| Evidence and Absence | `panel-radiocarbon-reservoir-old-wood-custody` | publication-ready |
| Evidence and Absence | `panel-microbotanical-starch-phytolith-custody` | publication-ready |
| Evidence and Absence | `panel-zooarchaeological-bone-modification-custody` | publication-ready |
| Evidence and Absence | `panel-soil-micromorphology-occupation-surface-custody` | publication-ready |
| Evidence and Absence | `panel-canopy-lidar-settlement-custody` | publication-ready |
| Evidence and Absence | `panel-geophysical-subsurface-survey-custody` | publication-ready |
| Evidence and Absence | `panel-archaeometallurgical-provenance-custody` | publication-ready |
| Evidence and Absence | `panel-archaeobotanical-macroremain-custody` | publication-ready |
| Evidence and Absence | `panel-ancient-pathogen-genome-custody` | publication-ready |
| Evidence and Absence | `panel-rock-surface-exposure-dating-custody` | publication-ready |
| Evidence and Absence | `panel-gobekli-tepe-partial-excavation-shelter-custody` | publication-ready |
| Evidence and Absence | `panel-catalhoyuk-partial-excavation-shelter-custody` | publication-ready |
| Urban Overbuild and Excavation Windows | `panel-templo-mayor-urban-overbuild-window-custody` | publication-ready |
| Urban Overbuild and Excavation Windows | `panel-london-mithraeum-relocation-reconstruction-custody` | publication-ready |
| Rock-Cut Landscapes and Erosion | `panel-petra-rock-cut-facade-erosion-custody` | publication-ready |
| Rock-Cut Landscapes and Erosion | `panel-lalibela-rock-cut-church-shelter-custody` | publication-ready |
| Surface Landscapes and Scale Mediation | `panel-nasca-geoglyph-scale-visibility-custody` | publication-ready |
| Surface Landscapes and Scale Mediation | `panel-uffington-white-horse-scale-visibility-custody` | publication-ready |
| Reconstruction and Modern Projection | `panel-lascaux-fragile-access-replica-custody` | publication-ready |
| Reconstruction and Modern Projection | `panel-chauvet-fragile-access-replica-custody` | publication-ready |
| Reconstruction and Modern Projection | `panel-palmyra-replica-reconstruction` | publication-ready |
| Reconstruction and Modern Projection | `panel-antikythera-reconstruction` | publication-ready |
| Reconstruction and Modern Projection | `panel-knossos-restoration-projection` | publication-ready |
| Monumental Sites and Contested Interpretation | `panel-great-zimbabwe-colonial-denial` | publication-ready |
| Monumental Sites and Contested Interpretation | `panel-mound-builder-denial-register` | publication-ready |
| Monumental Sites and Contested Interpretation | `panel-benin-bronzes-dispersed-custody` | publication-ready |
| Reconstruction and Modern Projection | `panel-parthenon-sculptures-dispersed-custody` | publication-ready |
| Lost Techniques and Broken Transmission | `panel-roman-concrete-partial-survival` | publication-ready |
| Lost Techniques and Broken Transmission | `panel-cahokia-without-collapse-theater` | publication-ready |
| Lost Techniques and Broken Transmission | `panel-tyrian-purple-partial-transmission` | publication-ready |
| Shipwrecks and Context Loss | `panel-uluburun-shipwreck-context-loss` | activation-ready working paper |
| Shipwrecks and Context Loss | `panel-kyrenia-reconstruction-context-loss` | activation-ready working paper |

## Review Roles

RUINS uses repository-local review roles:

- Fragment Custodian,
- Absence Historian,
- Reconstruction Skeptic,
- Cultural Memory Reader,
- Sibling Boundary Editor,
- Paper Editor,
- Safety Editor.

## Method Evolution

RUINS follows the portfolio pattern: build the artifact, review it against an
explicit rubric, then let repeated findings improve the process.

- [`docs/rubric-amendments.md`](docs/rubric-amendments.md) defines the
  forward-only amendment policy.
- [`docs/capability-expansion.md`](docs/capability-expansion.md) tracks
  case-derived RUINS capabilities and repeated rubric gaps.
- [`docs/capability-promotion-packet.md`](docs/capability-promotion-packet.md)
  gives ASHLAR the fast-promotion checklist for new capabilities.
- [`docs/capability-roadmap.md`](docs/capability-roadmap.md) identifies the
  next papers and method artifacts that promote active capabilities to core.
- [`skills/README.md`](skills/README.md) operationalizes paper scaffolding,
  source locking, role review, and method evolution.

## Non-Goals

- No claims of secret history, suppressed proof, or sensational certainty.
- No treasure-hunting instructions or site-looting enablement.
- No real-time archaeological advice or conservation guidance.
- No fictional canon catalog unless clearly marked as downstream game/world
  design material.
- No implementation work until the paper method survives at least five unlike
  case studies.
