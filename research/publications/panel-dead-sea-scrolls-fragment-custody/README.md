# Dead Sea Scrolls Fragment Custody

Paper `panel-dead-sea-scrolls-fragment-custody` in the [RUINS](../../) module.

## Read

- [**paper.md** - the paper draft](paper.md)
- [**plan.md** - paper plan](plan.md)
- [**source-map.md** - source and trace work surface](source-map.md)
- [**source-lock.md** - locked source uses and claim limits](source-lock.md)
- [**page-anchor-audit.md** - page anchors for current stable claims](page-anchor-audit.md)
- [**named-fragment-anchor-audit.md** - named image/metadata anchors](named-fragment-anchor-audit.md)
- [**authenticity-report-audit.md** - MOTB authenticity-report custody](authenticity-report-audit.md)
- [**edition-custody-gate.md** - future exact-text custody gate](edition-custody-gate.md)
- [**edition-acquisition-packet.md** - lawful edition-page acquisition targets](edition-acquisition-packet.md)
- [**publication-scope-gate.md** - publication-ready non-reading boundary](publication-scope-gate.md)
- [**chronology.md** - discovery, publication, reconstruction, and authenticity layers](chronology.md)
- [**claim-audit.md** - fragment, edition, translation, and authenticity claim checks](claim-audit.md)
- [**refusal-register.md** - claims this paper blocks](refusal-register.md)
- [**cultural-memory.md** - scrolls, scripture, secrecy, and authenticity afterlife](cultural-memory.md)
- [**sibling-boundary.md** - repo boundary and handoff register](sibling-boundary.md)
- [**_panel.yaml** - review panel](./_panel.yaml)
- [**reviews/SCORECARD.md** - first ASHLAR scorecard](reviews/SCORECARD.md)
- [**reviews/ROLE_REVIEW.md** - role panel review](reviews/ROLE_REVIEW.md)

## Navigate

[up publications](../) | [up research](../../) | [up repo root](../../../)
