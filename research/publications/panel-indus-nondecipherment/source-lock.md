# Source Lock: Indus Script Non-Decipherment

**Owner:** PALIMPSEST
**Status:** first source lock
**Date:** 2026-07-04

This file locks the first source set for the working paper. Locking a source
means RUINS may use it for a bounded claim. It does not mean the source's
interpretation is accepted as final.
...
|---|---|---|---|
| Undecipherment constraints | The corpus is small for decipherment, lacks a bilingual text, and lacks definite knowledge of the underlying language. | S1 | Corroborated |
| M77 corpus values | The M77 concordance records 417 unique signs in 3,573 lines across 2,906 texts. | S1 | Corroborated as source-specific catalog value |
| EBUDS value | The Yadav/Rao n-gram paper's pruned EBUDS dataset contains 1,548 texts. | S1 | Observed within source method |
...
1. Use exact numeric values only with source labels, for example "in M77 as used
   by Yadav et al."
2. Do not combine sign counts from different catalog traditions into a single
   universal count.
3. Treat statistical structure as structure, not translation.
4. Treat both linguistic and non-linguistic classifications as live
